...
"""Base class for substeps in the Model Chemistry step."""
...
class Substep(seamm.Node):
    """A base class for substeps in the Model Chemistry subflowchart.

    The top-level ModelChemistry step iterates over the substeps
    of its subflowchart and calls each substep's `run()` method. Each substep
    is responsible for preparing its own input, invoking Model Chemistry,
    ...
    * `version` / `git_revision` - delegated to the plug-in package so that
    ...
    @property
    def version(self):
        """The semantic version of the model_chemistry_step package."""
        return model_chemistry_step.__version__

    @property
    def git_revision(self):
        """The git revision of the model_chemistry_step package."""
    ...
    @property
    def options(self):
        """The Model Chemistry options, delegated to the parent step."""
        return self.parent.options

    @property
    def input_only(self):
        """Whether to write the input only, not run Model Chemistry."""
